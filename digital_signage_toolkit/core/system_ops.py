...
class SystemOperations:
    ...
    def set_hostname(self, new_hostname: str) -> bool:
        ...
        from ..utils.validators import sanitize_hostname
...
"""
                    ...
                    for line in result.stdout.split('\n'):
                        if line.strip().startswith((' ', '\t')):
                            ...
                            for match in matches:
                                from ..utils.validators import validate_resolution
                                if validate_resolution(match):
                                    resolutions.add(match)
                    if resolutions:
                        return sorted(resolutions, key=lambda x: int(x.split('x')[0]) * int(x.split('x')[1]), reverse=True)
            except FileNotFoundError:
                pass
    ...
    @log_operation_errors("TOGGLE_TV_POWER", False)
    def toggle_tv_power(self, power_on: bool) -> bool:
        """Toggle physical TV power using HDMI-CEC."""
        # Ensure cec-utils is installed
        if shutil.which('cec-client') is None:
            self.install_packages(['cec-utils'])

        cmd_str = "on 0" if power_on else "standby 0"
        
        # Use a bash pipe with cec-client
        try:
            result = self.sudo.run_command(
                ['bash', '-c', f'echo "{cmd_str}" | cec-client -s -d 1'],
                timeout=15
            )
            if result.returncode != 0:
                self.logger.log_error(
                    RuntimeError(f"cec-client failed: {result.stderr}"),
                    "TOGGLE_TV_POWER"
                )
            return result.returncode == 0
        except Exception as e:
            self.logger.log_error(e, "TOGGLE_TV_POWER")
            return False

    @log_operation_errors("CONFIGURE_STATIC_IP", False)
    def configure_static_ip(self, interface: str, ip: str, gateway: str, dns: str) -> bool:
        """Configure interface with static IP using nmcli."""
        try:
            from ..utils.validators import validate_ip_address
            if not validate_ip_address(ip.split('/')[0]) or not validate_ip_address(gateway):
                return False
            ...
            for cmd in commands:
                result = self.sudo.run_command(cmd, timeout=10)
                if result.returncode != 0:
                    self.logger.log_error(
                        RuntimeError(f"nmcli failed: {result.stderr}"),
                        "CONFIGURE_STATIC_IP"
                    )
                    return False
            return True
        except Exception as e:
            self.logger.log_error(e, "CONFIGURE_STATIC_IP")
            return False

    @log_operation_errors("CONFIGURE_DHCP", False)
    def configure_dhcp(self, interface: str) -> bool:
        """Configure interface to use DHCP using nmcli."""
        try:
            commands = [
            ...
            for cmd in commands:
                ...
                if result.returncode != 0:
                    self.logger.log_error(
                        RuntimeError(f"nmcli DHCP failed: {result.stderr}"),
                        "CONFIGURE_DHCP"
                    )
                    return False
            return True
        except Exception as e:
            self.logger.log_error(e, "CONFIGURE_DHCP")
            return False
